...
def pseudo_sdc(df=None,
    ...
    def m_qcrsc(x, y, r):
        ...
        m_qc["rsd_tqc_robust"] = m_qc["area_corrected"].mad() / abs(
            m_qc["area_corrected"].median()
        )
        m_qc["rsd"] = m_qc["area"].std() / abs(m_qc["area"].mean())
        m_qc["rsd_tqc"] = m_qc["area_corrected"].std() / abs(
            m_qc["area_corrected"].mean()
        )
    ...
    def fun_within(x):
        ...
        sub_df["class"] = np.where(
            sub_df["sample"].str.contains(qc_label), "QC", "Sample"
        )
        sub_df["my_mad"] = sub_df.groupby(["batch", "compound"])["area"].transform(
            lambda x: x.mad()
        )
        sub_df["median_area"] = sub_df.groupby(["batch", "compound"])["area"].transform(
            "median"
        )
        ...
        sub_df["area_tmp"] = np.where(
            (sub_df["area"] >= sub_df["high_thresh"])
            | (sub_df["area"] <= sub_df["low_thresh"]),
            np.nan,
            sub_df["area"],
        )
        sub_df["area_imputed"] = sub_df.groupby(["batch", "compound"])[
            "area_tmp"
        ].transform(lambda x: x.interpolate(method="linear", limit_area="inside"))
        m = sub_df[sub_df["batch"].isin(train_batch)].copy()
        m["my_mad"] = m.groupby(["batch", "compound"])["area"].transform(
            lambda x: x.mad()
        )
        ...
        m["area_tmp"] = np.where(
            (m["area"] >= m["high_thresh"]) | (m["area"] <= m["low_thresh"]),
            np.nan,
            m["area"],
        )
        ...
        vals_keep = (
            np.ceil(vals_keep["pool_rank"].agg(["min", "max"]) / quantile_increment)
            * quantile_increment
        )
